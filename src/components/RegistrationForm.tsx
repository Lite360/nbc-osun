...
interface RegistrationFormProps {
  locationVerification: LocationVerificationResult | null;
  venue: VenueSettings;
  onSuccess: () => void;
}
...
export const RegistrationForm: React.FC<RegistrationFormProps> = ({
  ...
  const isFormValid = () => {
    return (
      ...
      idFile !== null &&
      consent &&
      locationVerification?.isVerified === true &&
      venue.is_active
    );
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!locationVerification?.isVerified) {
      Swal.fire({
        ...
        text: 'You must be physically present at the designated registration venue to submit your application.',
    ...
    try {
      ...
      const newRegistration = apiService.createRegistration({
        ...
        location_accuracy: locationVerification.distanceMeters,
        user_lat: locationVerification.userLat,
        user_lng: locationVerification.userLng,
      });
  ...
  const isLocationRestricted = !locationVerification?.isVerified || !venue.is_active;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                type="text"
                required
                disabled={isLocationRestricted || submitting}
          ...
          <div className="bg-slate-50 border border-dashed border-slate-300 rounded-lg p-4 text-center">
            ...
            <label
              ...
              className={`cursor-pointer inline-flex flex-col items-center justify-center ${
                isLocationRestricted ? 'opacity-50 cursor-not-allowed' : ''
              }`}
            >
        ...
        <div className="pt-2 border-t border-slate-100">
          <label className="flex items-start space-x-3 cursor-pointer">
            ...
            <span className="text-xs text-slate-600 leading-relaxed">
              I hereby declare that all information provided is accurate and true. I authorize NBC Osun to process my personal details and location data solely for corps member verification purposes.
            </span>
          </label>
        </div>
